...
/*   Updated: 2023/10/10 16:06:55 by avassor          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../inc/raycast.h"

void	comp_horiz(t_raydata *data, t_h *h, int y)
{
	t_color	color;
	int		x;
	...
	while (x < WIDTH)
	{
		...
		h->tx = (int)(data->texture[FLOOR]->width * (h->floorx - h->cellx))
			& (data->texture[FLOOR]->width - 1);
		h->ty = (int)(data->texture[FLOOR]->height * (h->floory - h->celly))
			& (data->texture[FLOOR]->height - 1);
		h->floorx += h->floorstepx;
		h->floory += h->floorstepy;
		color = data->texture[FLOOR]->texture
		[data->texture[FLOOR]->width * h->ty + h->tx];
		color.color = (color.color >> 1) & 8355711;
		my_mlx_pixel_put(data->img_buffer, color, (t_int_point){x, y});
		color = data->texture[CEIL]->texture
		[data->texture[CEIL]->width * h->ty + h->tx];
		color.color = (color.color >> 1) & 8355711;
		my_mlx_pixel_put(data->img_buffer, color,
			(t_int_point){x, HEIGHT - y - 1});
		x++;
	}
}
