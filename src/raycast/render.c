        ...
        for (int j = 0; j < raydata->map_width;j ++) {
            if (raydata->map[i][j] == 1) {
                fill_rectangle(raydata, grid_color,
                               (t_point) {(double)(j * grid_size_x) + OFFSET_MAP_X, (double)(i * grid_size_y) + OFFSET_MAP_Y},
                               (t_point) {(double)((j + 1) * grid_size_x) + OFFSET_MAP_X, (double)((i + 1) * grid_size_y)+OFFSET_MAP_Y});
            }
