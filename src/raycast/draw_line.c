...
*******************************************************************************/
static void
draw_line_low(t_raydata *raydata, t_color color, t_point from, t_point to) {
	int y_inc;
	int D;
	...
	for (coord.x = (int) from.x, coord.y = (int) from.y;
		 coord.x <= (int) (to.x); coord.x++) {
...
static void
draw_line_high(t_raydata *raydata, t_color color, t_point from, t_point to) {
	int x_inc;
	int D;
	t_int_point coord;
	t_int_point delta_coord;
	...
	for (coord.y = (int) (from.y), coord.x = (int) (from.x);
		 coord.y <= (int) (to.y); coord.y++) {
		my_mlx_pixel_put(raydata->img_buffer, color, coord);
		if (D > 0) {
			coord.x += x_inc;
			D += 2 * (delta_coord.x - delta_coord.y);
